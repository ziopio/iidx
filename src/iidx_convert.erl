...
read_or_generate_preview(BMSfolder) ->
    FullWildcard = binary_to_list(filename:join(BMSfolder,  <<"preview.*">>)),
    PreviewBin = case filelib:wildcard(FullWildcard) of
        [File] ->
            convert_audiofile_to_wav(File);
        _ ->
            iidx_cli:warn("No one or multiple preview found, generating one..."),
            generate_preview()
    end,
    #{
        data => PreviewBin,
...
generate_preview() ->
    SilentPreview = filename:join(iidx_cli:get_temp_dir(), "dummy_preview.wav"),
    ...
    Binary = iidx_cli:read_file(SilentPreview),
    file:delete(SilentPreview),
    Binary.
